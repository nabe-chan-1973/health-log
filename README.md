...
朝の血圧（最高・最低）・脈拍と、夜の体重・体脂肪率を記録するオフライン対応の PWA です。
データは端末内（IndexedDB）にのみ保存され、サーバーには送信されません。

## 機能

- **入力**: 日付・血圧（最高/最低）・脈拍・体重・体脂肪率・メモを記録（1日1レコード）
...
- 血圧計に保存された測定データが `0x2A35` の通知（Indicate）で届き、**受信した記録を日付ごとに一覧へ自動保存**します（同じ日は最新の測定を採用）。
- 単位が kPa の機器は mmHg に自動換算します。
- 機器が見つからない・対応していない場合は、そのまま手入力してください。

### A&D UA-651BLE Plus での接続手順（実機で確認済み）

この機種はボンディング（暗号化ペア）が必要で、**Windows・Android とも先に OS 側でペアリングしておく**と安定します。

1. **血圧計をペアリングモードにする**（本体ボタン長押しで「P」/ Bluetooth マークが点滅。詳細は付属マニュアル）
2. **OS の Bluetooth 設定でペアリング**しておく
   - Android(Pixel): 位置情報を ON にし、Chrome に「付近のデバイス」/位置情報の権限を許可 → 設定でデバイスを追加
   - Windows: 設定 → Bluetooth とデバイス → デバイスの追加
   - ※ ペアリングしていないと、アプリの選択ダイアログに機器が出てこないことがあります
3. **血圧計で新しく 1 回測定する**（UA-651BLE は「まだ送っていない記録」だけを送信します）
4. 転送待ち（点滅）中に、アプリで **「📶 血圧計から取り込む」→ 機器を選択**
5. 「◯件を一覧に保存しました ✓」と出たら「一覧」タブで確認
